...
db.collection('users').find( 

  {
    ...
    $or: [ { 'senderdetails.SendersFirstName' : { $regex: first3 , $options:"$i" } }, { 'senderdetails.SendersLastName' :  { $regex: last3 ,$options:"$i" }  } , {"senderdetails.Mobile" : req.query.mobile }] 
...

{
 ...
 'customerdetails.Email' : req.query.mobile ,"_id": { $ne: arg } 
...
waterfall([
  ...
  function(callback){
   ...
   db.collection('users').findOne( {"customerdetails.Email" :req.query.mobile} ,function(e, results){
...
db.collection('users').findOne( {"username" : req.query.username} ,function(e, results){
   ...
   if (results) {
        ...
        var response = {"ERROR":"That senders username is already being used."};
...
function(err, results) {
          ...
          var response = {"ERROR":"","TYPE":req.query.user_type,"_id":results.ops[0]._id};
